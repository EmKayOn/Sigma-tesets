		...
		for (int j = 0; j < 2; j++)
			for (int k = 0; k < 2; k++)
				for (int l = 0; l < 2; l++)
					for (int m = 0; m < 2; m++)
						for (int n = 0; n < 2; n++) {
							int p = (i ? sb.second : sb.first) * (j ? sr.second : sr.first) * //Possibilities
								(k ? sd.second : sd.first) * (l ? tb.second : tb.first) *
								(m ? tr.second : tr.first) * (n ? td.second : td.first);
							if (!p) continue; //If Possibility 
							if ((i == 0 || l == 0) && j == 1 && m == 1 && (k == 0 || n == 0)) x[0] += p / q; //BLack
							else if ((i == 0 || l == 0) && j == 1 && m == 1 && k == 1 && n == 1) x[1] += p / q; //Blue
							else if (i == 1 && l == 1 && j == 1 && m == 1 && (k == 0 || n == 0)) x[2] += p / q; //Chocolate
							else if (i == 1 && l == 1 && j == 1 && m == 1 && k == 1 && n == 1) x[3] += p / q; //Lilac
							else if (j == 0 && m == 0 && (k == 0 || n == 0)) x[4] += p / q; //Red
							else if (j == 0 && m == 0 && k == 1 && n == 1) x[5] += p / q; //Cream
							else if ((i == 0 || l == 0) && j == 1 && m == 0 && (k == 0 || n == 0)) {
								x[0] += p / (q * 2);  //BLack /(Total * 2)
								x[6] += p / (q * 2); //Black-Red Tortie /(Total * 2)
							} else if ((i == 0 || l == 0) && j == 1 && m == 0 && k == 1 && n == 1) {
								x[1] += p / (q * 2); //Blue
								x[7] += p / (q * 2); //Blue-Cream Tortie
							} else if (i == 1 && l == 1 && j == 1 && m == 0 && (k == 0 || n == 0)) {
								x[2] += p / (q * 2); //Chocolate
								x[8] += p / (q * 2); //Chocolate-Red Tortie
							} else if (i == 1 && l == 1 && j == 1 && m == 0 && k == 1 && n == 1) {
								x[3] += p / (q * 2); //Lilac
								x[9] += p / (q * 2); //Lilac-Cream Tortie
							} else if ((i == 0 || l == 0) && j == 0 && m == 1 && (k == 0 || n == 0)) {
								x[4] += p / (q * 2); //Red
								x[6] += p / (q * 2); //Black-Red Tortie
